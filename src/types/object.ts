...
const e: {b: number} = { // 객체를 const로 선언해도 b를 12로 추론하지 않는다. 객체는 변경될수 있는 값을 가지기 때문이다.
    b: 12
} // {b: number}
...
class Person {
    constructor (
        public firstName: string, // public 은 this.firstName = firstaName을 단축한 것
...
let aaa: {
    b: number // number타입의 프로퍼티 b를 포함한다.
    c?: string // string 타입의 프로퍼티 c를 포함할 수도 있다.
    [key: number]: boolean // boolean 타입을 갖는 number 타입의 프로퍼티 여러개를 포함할 수 있다.
}
